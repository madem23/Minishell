...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/30 13:05:11 by mdemma            #+#    #+#             */
/*   Updated: 2022/11/30 13:05:13 by mdemma           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	count_unparsed_word(t_tree *branch)
{
	...
	while (tmp && tmp->index <= branch->end_index)
	{
		if ((tmp->type == TK_WORD || tmp->type == TK_DOLLAR || tmp->type == TK_EQUAL
			||  tmp->type == TK_QUOTE) && tmp->parsed == false)
			count++;
		tmp = tmp->next_token;
	}
	return (count);
...
void	filling_cmd_tab(t_tree *branch, int size)
{
	...
	while (tmp && tmp->index <= branch->end_index)
	{
		if ((tmp->type == TK_WORD || tmp->type == TK_DOLLAR || tmp->type == TK_QUOTE)
			 && (tmp->parsed == false && i == 0))
		{
			tmp->parsed = true;
			branch->exec_args[i++] = ft_strdup(tmp->value);
		}	 
		if ((tmp->type == TK_WORD || tmp->type == TK_EQUAL || tmp->type == TK_DOLLAR || tmp->type == TK_QUOTE)
			 && (tmp->parsed == false && i > 0))
		{
			tmp->parsed = true;
			branch->exec_args[i++] = ft_strdup(tmp->value);
